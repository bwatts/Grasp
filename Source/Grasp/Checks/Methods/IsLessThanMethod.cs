...
{
	/// <summary>
	/// Produces check rules for the <see cref="Checkable.IsLessThan{T}"/> method
	/// </summary>
	public sealed class IsLessThanMethod : ComparisonMethod
	{
		...
		public IsLessThanMethod(object value)
		{
			_value = value;
		}

		/// <summary>
		/// Gets the <see cref="Checkable.IsLessThan{T}"/> method
		/// </summary>
		...
		/// <returns>The <see cref="Checkable.IsLessThan{T}"/> method</returns>
		protected override MethodInfo GetMethod(Type targetType, Type checkType)
		{
			return typeof(Checkable).GetMethod("IsLessThan").MakeGenericMethod(targetType);
		}
